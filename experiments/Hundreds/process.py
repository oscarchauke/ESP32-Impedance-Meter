...
def process_all_files():
    ...
    tests = [f for f in os.listdir() if os.path.isdir(os.path.join(f)) & os.path.join(f).startswith("F")]
...
def check_frequency_domain(test, file):
    ...
    if os.path.exists(file_name):
        data = pd.read_csv(file_name)
        im_tools.plot_compare_fft(np.fft.fft(data['Voltage']), np.fft.fft(data['Current']))
